...
import UniformListMenu from '../uniform/list-menu'
...
import {createApiEnhancer, useContextualEditing} from '@uniformdev/canvas-react'
import {ManifestClient} from '@uniformdev/context/api'
import Cookie from 'cookie'
import createUniformContext from '../../utils/uniform/context'
import customFetch from '../../utils/uniform/customFetch'
...
const App = (props) => {
    ...
    clientContext = createUniformContext(manifest, cookieValue)
...
App.getProps = async ({api, res, req}) => {
    ...
    const context = createUniformContext(manifest, cookieValue)
    const originalCookies = Cookie.parse(cookieValue || '')

    await context.update({
        cookies: originalCookies,
        url: new URL(`http://salesforce.com${req.originalUrl}`)
    })

    const serialized = serializeCookie(context.storage.data)

    const updatedCookieValue = Cookie.serialize(UNIFORM_DEFAULT_COOKIE_NAME, serialized)
...
`
    ...
    const navComposition = await getCompositionBySlug('/header-nav', preview)
    ...
    return {
        ...
        manifest,
        cookieValue: updatedCookieValue,
        navComposition
    }
}

// These methods below should be exported from the context package and removed from here.
const TYPE_SEP = '~'
const PAIR_SEP = '!'
const KV_SEP = '-'
